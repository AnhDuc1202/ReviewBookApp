...
namespace ReviewBook.API.Controllers
{
    ...
    public class PublisherController : ControllerBase
    {
        ...
        public ActionResult<Publisher> Post([FromBody] CreatePublisherDTOs value)
        {
            ...
            if (acc.ID_Role == 1)
            {
                var check = _PublisherService.CheckName(value.Name);
                if (check != null)
                    return Problem("Đã tồn tại nhà xuất bản mang tên này",
        ...
        public ActionResult<Publisher> Put(int id, [FromBody] UpdateInforPublisherDTOs value)
        {
            ...
            if (acc.ID_Role == 1)
            {
                ...
                if (check != id && check != null)
                    return Problem("Đã tồn tại nhà xuất bản khác mang tên này",
                    statusCode: (int)HttpStatusCode.BadRequest);
